...
def CompareLayoutSegments(img, segs_opt, depth_opt, polys_opt, segs_noopt, depth_noopt, polys_noopt, segs_gt, label, name):
    '''! saves the prediction as segments on the original image next to the original image and saves it into the results folder 

    @param img: original image
    @param segs_opt: optimized segments 
    @param depth_opt: optimized depth map 
    @param polys_opt: optimazed polylines for the 2D and 3D visualization of the original NonCuboidRoom  
    @param segs_noopt: non-optimized segments
    @param depth_noopt: non-optimized depth map
    @param polys_noopt: non-optimazed polylines for the 2D and 3D visualization of the original NonCuboidRoom
    @param segs_gt: segements ground truth
    @param label: labels of the different segments to get the colorization 
    @param name: Name that will be used saving the picture
    '''
    palette = [
    ...
    cv2.imwrite(f'results/{name}_comparison_segs.png',
                np.concatenate([img[:360], img3[:360]], axis=1) * 255)

def CompareLayoutLines(img, segs_opt, depth_opt, polys_opt, segs_noopt, depth_noopt, polys_noopt, segs_gt, label, name):
    '''! Saves the prediction as lines on the original image next to the original image and saves it into the results folder 
    ...
    cv2.imwrite(f'results/{name}_comparison_lines.png',
                np.concatenate([img[:360], img3[:360]], axis=1) * 255) #img1[:360], img2[:360],

def CompareLayoutDark(img, segs_opt, depth_opt, polys_opt, segs_noopt, depth_noopt, polys_noopt, segs_gt, label, name):
    '''! saves the prediction as lines on a black background next to the original image and saves it into the results folder 
    ...
    cv2.imwrite(f'results/{name}_comparison_lines_dark.png',
                np.concatenate([img[:360],img3[:360]], axis=1) * 255)

def Display2DSeg(img, segs_opt, depth_opt, polys_opt, segs_noopt, depth_noopt, polys_noopt, segs_gt, label, name):
    '''! saves the prediction as segments on the original image and saves it into the results folder 
    ...
    cv2.imwrite(f'results/{name}_segments.png',
                np.concatenate([img3[:360]], axis=1) * 255)

def Display2DLines(img, segs_opt, depth_opt, polys_opt, segs_noopt, depth_noopt, polys_noopt, segs_gt, label, name):
    '''! saves the prediction as lines on the original image and saves it into the results folder 
    ...
    for i in range(len(color3)):
        ...
        #img3 = img3 * (1 - alpha_fill) + alpha_fill * color
        img3 = img3 * (1 - alpha_edge) + alpha_edge * color
        #json_object = json.dumps(alpha_edge.tolist())
        #with open("sample.json", "w") as outfile:
        #    outfile.write(json_object)

        bound_opt[alpha_edge[:,:,0]>0]=1.

    cv2.imwrite(f'results/{name}_lines.png',
                np.concatenate([img3[:360]], axis=1) * 255)

def Display2DLinesDark(img, segs_opt, depth_opt, polys_opt, segs_noopt, depth_noopt, polys_noopt, segs_gt, label, name):
    '''! saves the prediction as lines on a black background and saves it into the results folder 
    ...
    cv2.imwrite(f'results/{name}_lines_dark.png',
                np.concatenate([img3[:360]], axis=1) * 255)

def displayNYU(img, segs_pred, segs_gt, label, iters, method=9, draw_gt=0):
